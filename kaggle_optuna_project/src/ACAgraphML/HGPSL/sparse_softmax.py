...
def _make_ix_like(batch):
    ...
    idx = [torch.arange(1, i + 1, dtype=torch.long, device=batch.device)
           for i in num_nodes]
...
class SparsemaxFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.where(
            output != 0, grad_input - v_hat[batch], grad_input)
...
if __name__ == '__main__':
    sparse_attention = Sparsemax()
    input_x = torch.tensor([1.7301, 0.6792, -1.0565, 1.6614, -
                           0.3196, -0.7790, -0.3877, -0.4943, 0.1831, -0.0061])
