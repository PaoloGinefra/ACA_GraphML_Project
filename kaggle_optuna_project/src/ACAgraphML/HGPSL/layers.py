...
# Try different import paths for compatibility with different PyG versions
try:
    from torch_geometric.nn.pool.topk_pool import topk, filter_adj
except ImportError:
    try:
        from torch_geometric.nn.pool import topk, filter_adj
    except ImportError:
        try:
            from torch_geometric.nn import topk, filter_adj
        except ImportError:
            # Fallback: implement basic versions if not available
            def topk(x, ratio, batch, min_score=None, tol=1e-7):
                ...
                cum_num_nodes = torch.cat(
                    [batch.new_zeros(1), torch.bincount(batch).cumsum(dim=0)])

                index = torch.arange(num_nodes, device=x.device)
                perm = []

                for i in range(batch_size):
                    start, end = cum_num_nodes[i], cum_num_nodes[i + 1]
                    batch_x = x[start:end]
                    batch_size_current = end - start

                    if ratio < 1.0:
                        k = max(1, int(ratio * batch_size_current))
                    else:
                        k = int(ratio)
                    k = min(k, batch_size_current)

                    _, batch_perm = torch.topk(
                        batch_x, k, largest=True, sorted=False)
                    perm.append(batch_perm + start)
            ...
            def filter_adj(edge_index, edge_attr, perm, num_nodes=None):
                ...
                node_idx[perm] = torch.arange(
                    perm.size(0), device=edge_index.device)
...
class TwoHopNeighborhood(object):
    def __call__(self, data):
        ...
        if edge_attr is None:
            ...
            try:
                # Try with fill_value parameter (older version)
                data.edge_index, edge_attr = coalesce(
                    edge_index, edge_attr, n, n, op='min', fill_value=fill)
            except TypeError:
                # Fallback for newer version without fill_value
                data.edge_index, edge_attr = coalesce(
                    edge_index, edge_attr, n, n)
                # Apply the fill logic manually
                edge_attr = torch.minimum(edge_attr, torch.tensor(
                    fill, device=edge_attr.device, dtype=edge_attr.dtype))
...
class HGPSLPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        if self.sample:
            ...
            hop_data = Data(x=original_x, edge_index=edge_index,
                            edge_attr=edge_attr)
